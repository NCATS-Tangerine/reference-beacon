...
if [ -f "./api/swagger-codegen-cli.jar" ]; then
...
if [ -f "./api/swagger-codegen-cli.jar" ]; then
	java -jar swagger-codegen-cli.jar generate -i $SPECIFICATION_FILE_PATH -l spring -o server1 --model-package $MODEL_PACKAGE --api-package $API_PACKAGE --additional-properties basePackage=$BASE_PACKAGE,configPackage=$CONFIG_PACKAGE
else
